...
num_iterations = 50
k = 5
poids_centrale = [1.5, 1.2]


def lloyd_algorithm(data, k, num_iterations):
    ...
    centroids[:, 0] = np.random.uniform(low=-1.8, high=-1.5, size=k)
    centroids[:, 1] = np.random.uniform(low=-1.8, high=-1.5, size=k)
    ...
    for i in range(num_iterations):
        ...
        for j in range(k):
            ...
            detection_radius = np.max(cost_func)
...
centroids, labels = lloyd_algorithm(data, k, num_iterations)
