...
robot_positions = [(0.2, 0.2), (0.8, 0.2), (0.2, 0.8), (0.8, 0.8)]
...
for i in range(len(robot_positions)):
    ...
    region = Voronoi(points[indices[:5]])
...
regions_plot = [ax.plot([], [], '-', color='gray')[0] for i in range(len(robot_positions))]
...
for i in range(n_iterations):
    for j in range(len(robot_positions)):
        ...
        robot_plots[j].set_data(robot_positions[j][0], robot_positions[j][1])
        region = cell_regions[j].point_region[trajectories[j][0]]
        vertices = [vor.vertices[v] for v in vor.regions[region] if v != -1]
        x, y = zip(*vertices)
        regions_plot[j].set_data(x, y)
    plt.pause(0.001)
